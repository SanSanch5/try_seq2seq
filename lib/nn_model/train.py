...
from configs.config import INPUT_SEQUENCE_LENGTH, MAX_NORMALIZED_TOKEN_LENGTH, TOKEN_REPRESENTATION_SIZE, DATA_PATH, SAMPLES_BATCH_SIZE, \
...
def get_training_batch(w2v_model, tokenized_dialog, token_to_index):
    ...
    for sents_batch in _batch(tokenized_dialog, SAMPLES_BATCH_SIZE):
        ...
        Y = np.zeros((len(sents_batch), MAX_NORMALIZED_TOKEN_LENGTH, token_voc_size), dtype=np.bool)
        for s_index, sentence in enumerate(sents_batch):
            ...
            for t_index, token in enumerate(sents_batch[s_index + 1][:MAX_NORMALIZED_TOKEN_LENGTH]):
